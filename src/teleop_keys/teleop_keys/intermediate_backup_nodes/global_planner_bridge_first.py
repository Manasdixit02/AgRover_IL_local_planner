...
class GlobalPlannerBridge(Node):
    ...
    - Publishes a relative 2D goal on /local_goal (Pose2D)
    """
    def __init__(self):
    ...
    def _planner_result_cb(self, future):
        ...
        # Choose a lookahead point
        idx = min(self.lookahead_idx, len(path.poses) - 1)
        global_target_pose: PoseStamped = path.poses[idx]
        
        global_target_pose.header.stamp = Time().to_msg()

        # Transform into robot frame using TF2 directly on PoseStamped
        try:
            ...
            self.get_logger().error(f"TF transform failed: {ex}")
            return
