...
class ILLocalPolicyNode(Node):
    ...
    def __init__(self):
        ...
        # NEW: keep raw (unnormalized) dx,dy for printing
    ...
    def goal_callback(self, msg: Pose2D):
        ...
        # NEW: store raw dx,dy for printing
    ...
    def control_loop(self):
        ...
        # NEW: print goal + cmd while running
        if self.latest_goal_raw is not None:
            dx, dy = self.latest_goal_raw
            self.get_logger().info(f"goal(dx,dy)=({dx:.3f},{dy:.3f}) | cmd(v,w)=({v:.3f},{w:.3f})")
        else:
            self.get_logger().info(f"goal(dx,dy)=(None) | cmd(v,w)=({v:.3f},{w:.3f})")
