...
# NEW: pygame for keyboard teleop toggle
...
class ILLocalPolicyNode(Node):
    ...
    def __init__(self):
        ...
        # Infer input dims from checkpoint
        lidar_in_dim = None
        if self.use_lidar:
        ...
        # ---------- NEW: teleop override mode ----------
        self.override_mode = False
        self.last_print = 0.0

        # Steps (match your teleop script defaults)
        ...
        # Control loop at 10 Hz (unchanged)
    ...
    def control_loop(self):
        # ---- NEW: handle pygame events + toggle override on 'o' ----
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                # keep node alive; just ignore window close to stay minimal
                pass
        ...
        if self.override_mode:
            ...
            if now - self.last_print > 0.2:
                if self.latest_goal_raw is not None:
                    ...
                    self.get_logger().info(f"[OVERRIDE] goal(dx,dy)=({dx:.3f},{dy:.3f}) | cmd(v,w)=({v:.3f},{w:.3f})")
                else:
                    self.get_logger().info(f"[OVERRIDE] goal(dx,dy)=(None) | cmd(v,w)=({v:.3f},{w:.3f})")
                self.last_print = now
            return

        # ---- Neural net mode (original behavior) ----
